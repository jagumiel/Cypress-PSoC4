...
#include "sclk.h"
...
#if defined(sclk__PC)
    #define sclk_SetP4PinDriveMode(shift, mode)  \
    do { \
        sclk_PC =   (sclk_PC & \
                                (uint32)(~(uint32)(sclk_DRIVE_MODE_IND_MASK << \
                                (sclk_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (sclk_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define sclk_SetP4PinDriveMode(shift, mode)  \
        do { \
            sclk_USBIO_CTRL_REG = (sclk_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(sclk_DRIVE_MODE_IND_MASK << \
                                    (sclk_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (sclk_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		sclk_SetP4PinDriveMode(sclk__0__SHIFT, mode);
    }
#endif
...
void sclk_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  sclk_INTCFG & (uint32)(~(uint32)position);
    sclk_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
